...
def load_df_data_v1():
    ...
    df = pd.read_csv(
        "app_user_keyword_sentiment/dataset/yahoo_news_preprocessed_sentiment.csv",
        sep="|",
    )
...
def get_daily_basis_sentiment_count(df_query, sentiment_type="pos", freq_type="D"):
    ...
    if sentiment_type == "pos":
        ...
        lambda_function = lambda senti: (
            1 if senti > 0.4 & senti < 0.4 else 0
        )  # 介於中間為中立
    else:
        return None
...
def filter_df_via_content(query_keywords, cond, cate, weeks):
    ...
    if cond == "and":
        ...
        condition = condition & df.content.apply(
            lambda text: all((qk in text) for qk in query_keywords)
        )  # 寫法:all()
    elif cond == "or":
        # query keywords condition使用者輸入關鍵字條件
        condition = condition & df.content.apply(
            lambda text: any((qk in text) for qk in query_keywords)
        )  # 寫法:any()
